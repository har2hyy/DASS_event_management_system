  ...
  return (
    ...
    <div className="w-full px-6 lg:px-12 py-8">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl p-8 mb-8 shadow-lg">
        ...
        <p className="text-indigo-100">Here's your event activity at Felicity 2026</p>
        <div className="mt-4">
          <Link to="/participant/events" className="inline-block bg-gradient-to-r from-green-400 to-emerald-500 text-white px-10 py-4 rounded-2xl text-lg font-bold hover:from-green-500 hover:to-emerald-600 transition shadow-xl">
            Browse Events →
          </Link>
        </div>
      </div>
      ...
      {list.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
          ...
          <p className="text-gray-500 text-lg font-medium">No events in this category yet.</p>
          {tab === 'Upcoming' && (
            <div className="mt-4">
              <Link to="/participant/events" className="inline-block bg-gradient-to-r from-green-400 to-emerald-500 text-white px-10 py-4 rounded-2xl text-lg font-bold hover:from-green-500 hover:to-emerald-600 transition shadow-xl">
        ...
        <div className="grid gap-4">
          {list.map((reg) => (
            <div key={reg._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    to={`/participant/events/${reg.event?._id}`}
                    className="inline-flex items-center gap-1 bg-gray-100 text-gray-600 px-3 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition whitespace-nowrap"
                  >
                    View Event
                  </Link>
                  <Link
                    to={`/participant/ticket/${reg._id}`}
                    className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-indigo-100 transition whitespace-nowrap"
                  >
                    🎟️ Ticket
                  </Link>
                </div>
