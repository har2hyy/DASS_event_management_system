...
const FeedbackSection = ({ eventId, canSubmit = false }) => {
 ...
 const [ratingFilter, setRatingFilter] = useState(0); // 0 = all, 1-5 = specific star
 ...
 <div
 key={star}
 className={`flex items-center gap-2 text-sm cursor-pointer rounded px-1 -mx-1 transition ${
 ratingFilter === star ? 'bg-indigo-500/20' : 'hover:bg-white/5'
 }`}
 ...
 ratingFilter === 0 ? 'bg-indigo-600 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/20'
 }`}
 >
 All
 </button>
 ...
 className={`px-2 py-1 rounded-lg text-xs font-medium transition flex items-center gap-0.5 ${
 ratingFilter === s ? 'bg-yellow-500/20 text-yellow-400' : 'bg-white/10 text-gray-400 hover:bg-white/20'
 }`}
 >
 ...
 .length === 0 ? (
 <p className="text-gray-500 text-sm text-center py-4">No feedback with {ratingFilter} star{ratingFilter > 1 ? 's' : ''}.</p>
 ) : (
 feedbacks
 .filter((fb) => ratingFilter === 0 || fb.rating === ratingFilter)
 .map((fb) => (
